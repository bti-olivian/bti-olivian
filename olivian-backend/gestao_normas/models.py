...
class Cliente(models.Model):
    ...
    cnpj = models.CharField(max_length=14, unique=True, help_text="Somente números")
    ...
    cep = models.CharField(max_length=8, help_text="Somente números")
    telefone = models.CharField(max_length=11, help_text="Somente números (incluir DDD)")
...
# MODELO 2: A NORMA
# Definições para campos com escolhas (mantenha as que precisar)
IDIOMA_CHOICES = [
    ('ingles', 'Inglês'),
    ('portugues', 'Português'),
]
...
class Norma(models.Model):
    ...
    norma = models.CharField(max_length=50, unique=True, help_text="O número ou código da norma, ex: GMW3059", default='N/A')
    revisao_atual = models.DateField(help_text="A versão da revisão atual, ex: 06/06/2000", null=False, blank=False)
    ...
    observacoes = models.TextField(max_length=200, blank=True, null=True, help_text="Observações internas sobre a norma.")
...
class NormaCliente(models.Model):
    ...
    def __str__(self):
        return f"Revisão da Norma '{self.norma.norma}' para o Cliente '{self.cliente.empresa}'"

# MODELO 4: O PERFIL DO USUÁRIO DO CLIENTE
...
# MODELO 5: HISTÓRICO DE REVISÃO SECUNDÁRIA
...
class RevisaoSecundariaHistorico(models.Model):
    ...
    def __str__(self):
        return f"Revisão Secundária: {self.tipo_revisao} em {self.data} para {self.norma.norma}"
    
class Notificacao(models.Model):
    # O usuário que receberá a notificação
    usuario = models.ForeignKey(User, on_delete=models.CASCADE, related_name='notificacoes')
    
    # O objeto ao qual a notificação se refere (neste caso, uma Norma)
    norma = models.ForeignKey(Norma, on_delete=models.CASCADE, related_name='notificacoes')
    
    # A mensagem da notificação
    mensagem = models.TextField()
    
    # Se a notificação já foi visualizada
    visualizada = models.BooleanField(default=False)
    
    # A data e hora da criação da notificação
    data_criacao = models.DateTimeField(auto_now_add=True)
    
    def __str__(self):
        return f"Notificação para {self.usuario.username} sobre {self.norma.norma}"
    
    # Em gestao_normas/models.py

class Comentario(models.Model):
    # ALTERADO: Agora o comentário pertence a uma relação Norma-Cliente
    ...
    def __str__(self):
        return f'Comentário de {self.usuario.username} em {self.norma_cliente.norma.norma}'
...
class Certificacao(models.Model):
    ...
    def __str__(self):
        return f"Certificação '{self.nome}' do cliente {self.cliente.empresa}"
